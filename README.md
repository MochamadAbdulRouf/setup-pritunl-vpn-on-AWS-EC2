...
## Pritunl AMI Automation using Packer 

Di real-world scenario, kita perlu setup `Pritunl` di environment yang berbeda beda. Setup secara manual akan menguras waktu yang sangat banyak. Kita bisa Automasi membuat custom AMI menggunakan `Packer` dan `Ansible`. 


Untuk menggunakan `packer`, Perlu menginstall nya di instance/local workstation. Berikut cara menginstal `Packer` dan `Ansible`.
...
Pastikan `instances` memiliki credentials atau role untuk membuat AMI. Tambahkan role dengan cara berikut 
```bash
EC2 Console → Instance → Actions → Security → Modify IAM Role → pilih packer-role → Update IAM Role 
```

![iam-role-for-ec2](./images/iam-role.png)
> disini saya menggunakan AWS Academy jadi saya menggunakan role yang sudah disediakan.


Jika installasi `Packer` dan `Ansible` sudah selesai, kita bisa clone repository github berisi project `Packer` khusus untuk automisasi pembuatan AMI.
```bash
git clone https://github.com/techiescamp/pritunl-vpn.git
```

Buka direktori packer
```bash
cd packer
```

Untuk initialize, Gunakan command berikut
```bash
packer init .
```

Untuk menjalankan konfigurasi, Gunakan command berikut
```bash
packer build vpn.pkr.hcl
```

Tunggu sekitar 10 - 13 minet, untuk pembuatan AMI. Jika tampilannya berhasil seperti gambar berikut, Cek AMI Section di EC2 Console.
![complete-ami-automisasi](./images/done-ami.png)


Di bagian AMI, kita bisa langsung membuat instance dengan klik button `Launch instance from AMI`, dengan begitu kita akan otomatis membuat instance yang sama dengan `Pritunl Server`.
![ami-section](./images/ami-section.png)